...
- Is there a better way to obtain/ get more consistently formatted dates?
- Better way to get version of reference genome used, primarily getting from filename and path
- See comments in the code chunk for filtering full list of bams to list of priority bams
...
Bam stats from CDK pipeline
```{r}
bamstats_cdk_df <- read_csv(here("raw-data/bamstats.csv"))
glimpse(bamstats_cdk_df)
```
...
priority_bams <- ftp_metadata %>%
    filter(
        ...
        ## Excluding PacBio CLR
        str_detect(filepath, "PacBio_MtSinai_NIST", negate = TRUE),
        ## Excluding older BGISEQ
        str_detect(filepath, "BGISEQ500", negate = TRUE)
    ) 
...
Adding bamstats to priority_bams
```{r}
priority_bams <- priority_bams %>% 
    left_join(bamstats_cdk_df)
glimpse(priority_bams)
```
...
```
### Cleaning up reference genome
```{r}
priority_bams_metadata_df <- priority_bams_metadata_df %>% 
    mutate(ref = case_when(is.na(ref) & !is.na(reference_genome) ~ reference_genome,
                           str_detect(filename, "GRCh38") ~ "GRCh38",
                           str_detect(filename, "GRCh37") ~ "GRCh37",
                           str_detect(filename, "hs37d5") ~ "hs37d5",
                           str_detect(filename, "chm13") ~ "CHM13", ## TODO will want to get version number
                           ## Hard coding references for files where reference is not indicated from the file path
                           # 6kb jump libraries
                           md5key == "f9376aa42d6e1362c6a52a74b83c8c92" ~ "GRCh37",
                           md5key == "5b8bf1cd5771689f8ebef8c428146b68" ~ "GRCh37",
                           md5key == "eff61acfb5863d6ff9897ef6953bfb9c" ~ "GRCh37",
                           md5key == "24a5f27ffb7ef46eb555517f21934436" ~ "GRCh37",
                           md5key == "1a2a69775e01a26d8618ab34432ba08e" ~ "GRCh37",
                           md5key == "f4e765afa04d4d4390cc20cc1ee69b69" ~ "GRCh37",
                           TRUE ~ ref))
```
...
```{r}
priority_bams_metadata_df <- priority_bams_metadata_df %>% 
    mutate(coverage_estimate = round(filesize / 3, 0))
```
...
```{r}
write_csv(bam_registry_tbl, "bam_metadata_20231030.csv")
```
